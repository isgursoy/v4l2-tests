...
class V4L2_Backend : public Capture_Backend
{
	...
	public:
#ifdef OCL_AVAILABLE
		typename Frame_Impl::Multiplanar_CL_Buffer share_frame_data(bool rw = false) override
		{
				typename Frame_Impl::Multiplanar_CL_Buffer cl_buffer;
				auto& _configuration_ = this->_configuration_;

				if(not _configuration_.opencl.is_able_to_import_dmabuf().empty())
				{
						for(int i = 0; i < _num_buffers_; ++i)
						{
								for(int j = 0; j < num_planes(); ++j)
								{
										const auto expbuf = this->_buffer_dma_fds_[i][j];
										cl_buffer.emplace_back(
												_configuration_.opencl.import_dmabuf(expbuf.first, rw, expbuf.second));
								}
						}
				...
				const typename Frame_Impl::Multiplanar_Buffer_View mmapped_data =
						this->get_frame_data();

				for(const auto& plane : mmapped_data)
				{
						cl_buffer.emplace_back(
								cl::Buffer(_configuration_.opencl.context(),
													 (rw ? CL_MEM_READ_WRITE : CL_MEM_READ_ONLY) | //
															 (try_mmapped ? CL_MEM_USE_HOST_PTR : CL_MEM_COPY_HOST_PTR),
													 plane.size() * sizeof(Data_Type),
													 plane.data()));

						if(cl_buffer.back().template getInfo<CL_MEM_SIZE>()
							 != plane.size() * sizeof(Data_Type))
						{
								std::cerr
										<< "Your environment is not suitable for using V4L2 dma buf to create "
											 "zero-copy CL buffer. Falling back to copy the data to the CL buffer."
										<< std::endl;
		...
		[[nodiscard]] std::vector<std::vector<size_t>> put_frame_data(
				std::vector<typename Frame_Impl::Multiplanar_CL_Buffer>& userspace_frames) override
		{
				...
				std::vector<Buffer_Mapping_Impl> mappings;
					 ...
					 or get_memory_mapping_type_v4l2() == V4L2_MEMORY_DMABUF)
				{
								...
								++userspace_frame_index)
						{
								...
								for(auto plane_index = 0; plane_index < planes_count; ++plane_index)
								{
										mappings.emplace_back(Buffer_Mapping_Impl{userspace_frame[plane_index],
																															_configuration_.opencl});

										std::copy(mmap_frame_buffer_view[plane_index].begin(),
															mmap_frame_buffer_view[plane_index].end(),
															mappings[plane_index].get_cpu_span().begin());
						...
						++userspace_frame_index)
				{
						...
						if(V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == this->_buffer_plane_type_)
						{
								...
								{
										const auto& plane_data = userspace_frame[plane_index];
										mappings.emplace_back(
												Buffer_Mapping_Impl{plane_data, _configuration_.opencl});

										planes[plane_index].m.userptr =
												ulong(mappings[plane_index].get_cpu_span().data());
						...
						{
								const auto& plane_data = userspace_frame[0];
								mappings.emplace_back(Buffer_Mapping_Impl{plane_data, _configuration_.opencl});

								buf.m.userptr = ulong(mappings[0].get_cpu_span().data());
								buf.length		= _v4l2_capture_format_.fmt.pix.sizeimage;
						}
